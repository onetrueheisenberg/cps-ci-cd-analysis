...
class DockerfileAnalyzer:
    ...
    def analyze_dockerfile(self, dockerfile_path: str) -> Dict[str, Any]:
        try:
            ...
            return {"error": f"Dockerfile not found: {dockerfile_path}", "scores": {}}
...
Dockerfile:\n```\n{dockerfile_content}\n```"""
        ...
        if raw_response.startswith("LLM API error"):
            return {
                ...
                "scores": {"size_score": 50.0, "estimated_wasted_space_kb": 0},
            }
        ...
        # Simple size score: assume 100% for negligible waste, scale down up to 90 points for large waste.
        size_score = max(0.0, 100.0 - min(90.0, wasted_kb / 50.0))

        scores = {"size_score": round(size_score, 1), "estimated_wasted_space_kb": round(wasted_kb, 2)}
        ...
        print(f"  [LLM Size Analysis] size_score={scores['size_score']} wasted_kb={scores['estimated_wasted_space_kb']}")

        return {"dockerfile_path": dockerfile_path, "llm_analysis": analysis, "scores": scores}
    ...
    def print_analysis_report(self, analysis_result: Dict[str, Any]) -> None:
        ...
        print("=" * 60)
        print(f"Size Score: {scores.get('size_score', 0):.1f}%")
        print(f"Estimated Wasted Space: {scores.get('estimated_wasted_space_kb', 0):.2f} kB")
